...
class ModelAgnosticMetaLearningModel(BaseModel):
    def __init__(
        ...
        self.save_after_epochs = args.save_after_epochs
        self.log_train_images_after_iteration = args.log_train_images_after_iteration
        self.report_validation_frequency = args.report_validation_frequency
        ...
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=self.meta_learning_rate)
    ...
    def evaluate(self, iterations = 5, epochs_to_load_from=None):
...
def run_omniglot(raw_data_address):
    omniglot_database = OmniglotDatabase(
        raw_data_address=raw_data_address,
        random_seed=47,
...
def run_mini_imagenet(raw_data_address):
    mini_imagenet_database = MiniImagenetDatabase(raw_data_address=raw_data_address, random_seed=-1)
