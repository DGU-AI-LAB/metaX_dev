...
class DataGenerator():
    ...
    def make_data_tensor(self, train=True):
        ...
        for _ in tqdm.trange(0, total_batch):
            ...
            self.labels = [li[0] for li in labels_and_images]
            self.filenames = [li[1] for li in labels_and_images]
            self.all_filenames.extend(self.filenames)
            self.all_labels.extend(self.labels)

        self.all_filenames = tf.convert_to_tensor(self.all_filenames)
        self.dataset = tf.data.Dataset.from_tensor_slices(self.all_filenames)
        self.dataset = self.dataset.map(
            lambda data_list: tf.py_function(_read_py_function, [data_list], [tf.int32]))
        self.dataset = self.dataset.map(_resize_function)

        self.task_image_size = (self.N * self.K) * 2
        self.batch_image_size = self.batch_size * self.task_image_size
        # print('Batching images')
        self.dataset = self.dataset.batch(self.batch_image_size)
    ...
    def next(self):
        ...
        query_task_idx = []
        idx_list = np.array(range(self.task_image_size))
        reshape_idx_list = idx_list.reshape(self.task_image_size // (self.K*2), self.K*2)
        ...
        for num_task in range(self.batch_size):
            ...
            for support_idx in support_task_idx:
                ...
                support_task_labels.append(self.labels[support_idx])
            for query_idx in query_task_idx:
                query_task_images.append(images[query_idx])
                query_task_labels.append(self.labels[query_idx])
        ...
        return zip(support_batch_data, support_batch_labels, query_batch_data, query_batch_labels)
...
if __name__ == '__main__':
    ...
    print(tf.stack(support_batch_data).shape)
    print(tf.stack(support_batch_labels).shape)
    print(tf.stack(query_batch_data).shape)
    print(tf.stack(query_batch_labels).shape)
