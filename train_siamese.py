from model.metric_based.Siamese import OmniglotModel, MiniImagenetModel, Siamese
...
if __name__ == '__main__':
    ...
    parser.add_argument('--meta_batch_size', type=int, default=50)  # 20.09.03
    ...
    # 학습을 위한 클래스를 생성합니다.
    siamese = Siamese(args, database, network_cls)
    ...
    siamese.meta_train(epochs = args.epochs)
    # epochs : 반복 횟수 type : int

    # meta_test 시 파라메터 없음
    siamese.meta_test(epochs_to_load_from=args.epochs)

    # 입력받은 파라미터를 통해 해당 epochs의 저장된 모델을 불러옵니다.
    siamese.load_model(epochs = args.epochs)
    # epochs : 몇번 학습한 모델을 불러올 지  type : int
    # None일 시 최종 학습한 모델을 불러옵니다.

    # 예측한 결과를 보여줍니다.
    print(siamese.predict_with_support(meta_test_path='/dataset/data/omniglot_test', epochs_to_load_from = 10))
