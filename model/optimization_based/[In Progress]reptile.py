...
class Reptile(MetaLearning):
    def __init__(
        ...
        self.model_before = self.network_cls(num_classes=self.n)
        self.model_before(tf.zeros(shape=(self.n * self.k, *self.database.input_shape)))
    ...
    def copy_meta_model(self, updated_model, model):
    ...
    # def create_meta_model(self, updated_model, model, gradient):
    def create_meta_model(self, updated_model, model):
        ...
        for i in range(len(model.layers)):
            if isinstance(model.layers[i], tf.keras.layers.Conv2D) or \
                    isinstance(model.layers[i], tf.keras.layers.Dense):
                updated_model.layers[i].kernel = model.layers[i].kernel + (updated_model.layers[i].kernel - model.layers[i].kernel) * self.lr_inner_ml
                # updated_model.layers[i].kernel = model.layers[i].kernel - self.lr_inner_ml * gradients[k]
                k += 1
                variables.append(updated_model.layers[i].kernel)
                updated_model.layers[i].bias = model.layers[i].bias + (updated_model.layers[i].bias - model.layers[i].bias) * self.lr_inner_ml
                ...
                if hasattr(model.layers[i], 'moving_variance') and model.layers[i].moving_variance is not None:
                    updated_model.layers[i].moving_variance.assign(model.layers[i].moving_variance)
                if hasattr(model.layers[i], 'gamma') and model.layers[i].gamma is not None:
                    updated_model.layers[i].gamma = model.layers[i].gamma + (updated_model.layers[i].gamma - model.layers[i].gamma) * self.lr_inner_ml
                    # updated_model.layers[i].gamma = model.layers[i].gamma - self.lr_inner_ml * gradients[k]
                    k += 1
                    variables.append(updated_model.layers[i].gamma)
                if hasattr(model.layers[i], 'beta') and model.layers[i].beta is not None:
                    updated_model.layers[i].beta = \
                        model.layers[i].beta + (updated_model.layers[i].beta - model.layers[i].beta) * self.lr_inner_ml
                    # updated_model.layers[i].beta = \
    ...
    def inner_train_loop(self, train_ds, train_labels, num_iterations=-1):
        if num_iterations == -1:
            ...
            # Copy the current model to self.model_before
            self.copy_meta_model(self.model_before, self.model)
            ...
            self.create_meta_model(self.updated_models[k], self.updated_models[k - 1])
            
            # Because this code is Reptile, below code is Imigrated from 'get_losses_of_tasks_batch'
            ...
            return self.updated_models[-1], loss
            ...
            # self.create_meta_model(self.updated_models[0], self.model, gradients)
            self.create_meta_model(self.updated_models[0], self.model)
            ...
            for k in range(num_iterations):
                ...
                self.create_meta_model(copy_model, copy_model)
    ...
    def meta_test(self, iterations = 5, epochs_to_load_from=None):
        ...
        for tmb, lmb in self.test_dataset:
            for task, labels in zip(tmb, lmb):
                ...
                updated_model, loss = self.inner_train_loop(train_ds, train_labels, iterations)
    ...
    def report_validation_loss_and_accuracy(self, epoch_count):
        ...
        for tmb, lmb in self.val_dataset:
            ...
            for task, labels in zip(tmb, lmb):
                ...
                updated_model, loss = self.inner_train_loop(train_ds, train_labels, self.num_steps_validation)
                # If you want to compare with MAML paper, please set the training=True in the following line
                # In that paper the assumption is that we have access to all of test data together and we can evaluate
    ...
    def get_losses_of_tasks_batch(self, inputs):
        ...
        updated_model, loss = self.inner_train_loop(train_ds, train_labels, -1)
    ...
    def meta_train_loop(self, tasks_meta_batch, labels_meta_batch, iteration_count):
        ...
        train_ds, val_ds, train_labels, val_labels = self.get_task_train_and_val_ds(tasks_meta_batch, labels_meta_batch)
        ...
        with tf.GradientTape(persistent=True) as outer_tape:
            tasks_final_losses = get_losses_of_tasks_batch((tasks_meta_batch, labels_meta_batch, iteration_count))
